...
# Wake words
WAKE_WORDS = ["alfred", "robot", "guy", "toad", "spinny"] # Keep your desired wake words

# Acknowledgement phrases
ACKNOWLEDGEMENTS = ["Hey!", "What's up?", "Howdy!", "What?", "Need something?", "Yes?", "I'm here."]
...
# VAD Settings
VAD_THRESHOLD = 0.012 # RMS energy threshold - may need tuning
...
def run_wakeword_server():
    ...
    clear_flag_file(REQUEST_AUDIO_CAPTURE_FLAG)
    ...
    try:
        while server_running:
            # 1. Check for robot-initiated listening request FIRST
            if os.path.exists(REQUEST_AUDIO_CAPTURE_FLAG):
                print("Wakeword Server: REQUEST_AUDIO_CAPTURE_FLAG detected. Initiating listening.")
                clear_flag_file(REQUEST_AUDIO_CAPTURE_FLAG)
                
                # Do NOT play acknowledgment sound - autonomous_control.py already said "I'm listening"
                do_listen_and_transcribe(stream, play_acknowledgment=False)
                
                # Clear audio buffer and continue to main loop
                audio_buffer_np = np.array([], dtype=np.float32)
                continue

            # 2. Normal wake word detection
                ...
                if rms >= VAD_THRESHOLD:
                    ...
                    try:
                        result = mlx_whisper.transcribe(
                            ...
                            path_or_hf_repo=ASR_MODEL_NAME,
                            language=LANGUAGE
                            # initial_prompt="Alfred" # Bias towards wake words if helpful
                        )
                        transcribed_text = result['text'].strip().lower()
                        
                        if transcribed_text:
                            print(f"Wakeword check - Transcribed: '{transcribed_text}'")
